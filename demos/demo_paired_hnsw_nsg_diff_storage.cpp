...
int main(
    ...
    // ========================================================
    // Verify independent vector storage
    //
    // Unlike an earlier version of this demo, HNSW and NSG do
    // NOT share a single IndexFlat here. Each index built its
    // own storage inside its own add() call above (own_fields
    // stays true for both, so each index's destructor cleans
    // up its own storage independently). This section just
    // confirms that separation holds and that both copies
    // ended up holding the same vectors.
    // ========================================================
    ...
    // The whole point: these must be two distinct IndexFlat
    ...
    std::cout
            << "\nSUCCESS: HNSW and NSG each own "
            << "separate, independent vector storage.\n";
    ...
    // copy inside each index's own IndexFlat storage, so these
    // can be freed freely once both build and query phases are
    // done. hnsw and nsg go out of scope right after this and
    // each destroys its own storage independently (own_fields
    // is true on both), so no manual storage cleanup is needed
    // here.
    // ========================================================
